...
struct ChordGuides: View {
    ...
    let chordsMinor = ["Cm","Dm","Em","Fm","Gm","Am","Bm"]
    let chordTypes = ["None","Chord7"]
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading) {
                ...
                VStack (alignment: .leading) {
                    Text("Use the hand you use to play major chords on Chord Zone to form a note.")
                ...
                VStack (alignment: .leading) {
                    Text("Use the hand you use to play minor chords on Chord Zone to form a note.")
                        .font(.custom("Inter", size: 23))
                        .fontWeight(.regular)
                        .foregroundStyle(Color("PrimaryBackground"))
                        .padding(.bottom)
                    ...
                    LazyVGrid(columns: columns1) {
                        ForEach(chordsMinor, id: \.self) { chord in
                            ChordCard(chord: chord)
                                .padding(.horizontal, 5)
                        }
                    }
